...
int main(int argc, char *argv[]) {
    ...
    Function *addFunc= createFunc(builder, "BaseClass::add"); 
    createCall(builder, "calladd",addFunc);
    ModuleOb->dump();
    auto H = TheJIT->addModule(std::move(ModuleOb));   
    
    int (BaseClass::*FP1)(int,int) = &BaseClass::add;
    uint64_t methodAddress= *(reinterpret_cast<uint64_t*>(&FP1));
    TheJIT -> addExternalFunction("BaseClass::add", methodAddress);
