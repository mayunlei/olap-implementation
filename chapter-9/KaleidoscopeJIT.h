...
public:
  PrototypeAST(const std::string &Name, std::vector<std::string> Args,std::vector<llvm::Type*> types)
      : Name(Name), Args(std::move(Args)),
  types(std::move(types)){}
  PrototypeAST(const std::string &Name, std::vector<std::string> Args)
      : Name(Name), Args(std::move(Args)){}

  llvm::Function *codegen(){ return NULL;}
...
private:
  ...
  std::map<std::string, uint64_t> ExternalSymbolAddress;
...
public:
  KaleidoscopeJIT()
      : Resolver(createLegacyLookupResolver(
            ...
            std::cout<<"find Symbol:"<<std::string(Name)<<std::endl;
              ...
              if (auto SymAddr = RTDyldMemoryManager::getSymbolAddressInProcess(
                ...
                auto itr = ExternalSymbolAddress.find(std::string(Name));
                if(itr != ExternalSymbolAddress.end()) {
  ...
  void addExternalFunction(const std::string & Name, uint64_t addr)
  {
      ExternalSymbolAddress.insert(make_pair(mangle(Name), addr));
  }
